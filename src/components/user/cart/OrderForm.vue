<template>
  ...
  <VForm v-slot="{ errors }" @submit="submit(order)" ref="form"
  class="text-primary-emphasis border-top border-primary mt-3 py-3">
    ...
    <div class="mb-3">
      ...
      >
        ...
        <option class="" :value="item.CityName" v-for="item in cityAreas" :key="item.CityName">{{ item.CityName }}</option>
      </VField>
    ...
    <div class="mb-3">
      ...
      >
        ...
        <option class="" :value="item.AreaName" v-for="item in selectedCityAreas" :key="item.A">{{ item.AreaName }}</option>
      </VField>
    ...
    <div class="mb-3">
      ...
      <ErrorMessage name="完整路名、巷弄號樓" class="invalid-feedback"></ErrorMessage>
    </div>
    <div class="mb-3">
...
export default {
  ...
  methods: {
    ...
    async fetchCityAreas() {
      const api = "https://raw.githubusercontent.com/donma/TaiwanAddressCityAreaRoadChineseEnglishJSON/master/CityCountyData.json"
